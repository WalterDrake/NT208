...
export const fetchBoardId = async (userId) => {
  try {
    const response = await axios.get(`${API_ROOT}/api/todolists/${userId}`)
    // Lưu ý: axios sẽ trả kết quả về qua property của nó là data
    return response.data
  } catch (err) {
...
export const moveCardToDifferentColumnAPI = async (updateData) => {
  const response = await axios.put(`${API_ROOT}/api/boards/supports/moving_card`, updateData)
  return response.data
}
...
export const updateColumnDetailsAPI = async (columnId, updateData) => {
  const response = await axios.put(`${API_ROOT}/api/columns/${columnId}`, updateData)
  return response.data
}

export const deleteColumnDetailsAPI = async (columnId) => {
  const response = await axios.delete(`${API_ROOT}/api/columns/${columnId}`)
  return response.data
}

/** Cards */
export const createNewCardAPI = async (newCardData) => {
  const response = await axios.post(`${API_ROOT}/api/cards`, newCardData)
  return response.data
}
