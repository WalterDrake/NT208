...
const Khoahocpage = () => {
  ...
  useEffect(() => {
    if (user.role === "admin") {
      ...
      courses
        .getCourseListTeacher(user._id)
        .then((res) => {
          if (res) setKhoahocs(res);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      <div className=" dimension">
        <div className=" ml-[1rem]">
          ...
          {user.role === "teacher" || user.role === "admin" ? (
            <CreateCourse isCourse={true} isStudy={false} user={user} />
          ) : null}
